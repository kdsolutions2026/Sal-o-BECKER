import logisticsData from "./logistics.json" with { type: "json" };
...
export function getPublicConfig() {
  ...
  if (!publicKey) {
    throw new Error("Defina MERCADO_PAGO_PUBLIC_KEY nas secrets do Supabase.");
  }
...
function inferWebhookUrl(requestUrl: string) {
  const configured = Deno.env.get("MERCADO_PAGO_WEBHOOK_URL")?.trim();
...
export function buildPaymentPayload(
  ...
  const payload: Record<string, unknown> = {
    ...
    notification_url: inferWebhookUrl(requestUrl),
  };
...
export async function createMercadoPagoPayment(payload: Record<string, unknown>) {
  const accessToken = Deno.env.get("MERCADO_PAGO_ACCESS_TOKEN")?.trim();
  if (!accessToken) {
    throw new Error("Defina MERCADO_PAGO_ACCESS_TOKEN nas secrets do Supabase.");
  }
