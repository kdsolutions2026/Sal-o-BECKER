(function () {
  ...
  const apiFetch = (key, options = {}) => {
    ...
    if (runtimeApiBaseUrl && url.startsWith(runtimeApiBaseUrl)) {
      if (!runtimeApiKey) {
        throw new Error("Defina apiKey em scripts/runtime-config.js antes de usar o checkout publicado.");
      }
  ...
  const showResult = ({ title, message, meta, pix }) => {
