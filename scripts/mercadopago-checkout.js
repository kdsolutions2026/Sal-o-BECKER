(function () {
  ...
  const runtimeApiProvider =
    String(runtime.apiProvider || "").trim().toLowerCase() ||
    (runtimeApiBaseUrl.includes("supabase.co") ? "supabase" : "cloudflare");
  const endpointMap = {
    config: {
  ...
  const apiFetch = (key, options = {}) => {
    ...
    if (runtimeApiProvider === "supabase" && runtimeApiBaseUrl && url.startsWith(runtimeApiBaseUrl)) {
      if (!runtimeApiKey) {
  ...
  const paymentAttemptId = (method, formData = {}) => {
    const buyer = getBuyerData();
    const fingerprint = JSON.stringify({
      method,
      items: checkoutState.items.map((item) => [item.id, item.qty]),
      shippingMode: checkoutState.shippingMode,
      shippingZip: checkoutState.shippingZip,
      cpf: buyer.cpf,
      email: buyer.email,
    ...
    const storageKey = `becker:payment-attempt:${(hash >>> 0).toString(16)}`;
          ...
          onSubmit: (formData) => {
            ...
            return apiFetch("processPayment", {
              ...
              headers: {
                ...
                "X-Idempotency-Key": paymentAttemptId("card", formData),
              },
              body: JSON.stringify({
                formData,
                buyer: getBuyerData(),
  ...
  async function createPixPayment() {
    ...
    const response = await apiFetch("processPayment", {
      ...
      headers: {
        "Content-Type": "application/json",
        "X-Idempotency-Key": paymentAttemptId("pix", { payment_method_id: "pix" }),
      },
      body: JSON.stringify({
