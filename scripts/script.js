...
const anaileTip = (product) => `
  <aside class="anaile-tip">
    <div class="anaile-head"><img src="${assetUrl("assets/img/anaile-avatar.webp")}" alt="Anaile, fundadora do Salão Becker"><div><span>✨ Dica da Anaile</span><strong>Recomendação profissional</strong></div></div>
...
const productCard = (product) => `
  <article class="product-card reveal visible" data-product-card data-card-link="${pageUrl(`produto.html?produto=${product.id}`)}" data-id="${product.id}" data-category="${product.category}" data-name="${product.name.toLowerCase()}">
    ...
    <div class="product-info"><span>${product.category}</span><h3><a href="${pageUrl(`produto.html?produto=${product.id}`)}">${product.name}</a></h3><p>${product.description}</p><div class="benefit-list">${product.benefits.map((benefit) => `<span>${benefit}</span>`).join("")}</div><div class="rating">★ ${product.rating} <small>(${product.reviews})</small></div><div class="price"><strong>${productPrice(product)}</strong>${product.oldPrice ? `<del>${money.format(product.oldPrice)}</del>` : ""}</div><div class="product-card-actions">${productAction(product)}<a class="btn small ghost dark" href="${pageUrl(`produto.html?produto=${product.id}`)}">Ver produto</a></div></div>
    ${anaileTip(product)}
  </article>`;
...
document.querySelector("[data-checkout-confirm]")?.addEventListener("click", () => {
  ...
  if (note) note.textContent = state.cart.length ? "Pedido registrado para revisão. Integração de pagamento pode ser conectada nesta etapa." : "Adicione produtos ao carrinho antes de confirmar.";
});
